...
import { isAuthenticated, logoutUser, syncLocalSessions } from "../../services/api";
    ...
    if (isAuthenticated()) {
      ...
      if (userStr) {
        try {
          ...
          setUserName(user.name || user.first_name || "Profil");
        } catch {
          setUserName("Profil");
        }

        // Auto-sync local sessions
        syncLocalSessions();
      }
    } else {
      setIsLoggedIn(false);
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode
      ...
      <div className="container mx-auto px-4 py-6">
        <header className="mb-8 relative">
          ...
          <div className="absolute top-0 left-0">
            ...
            {location.pathname === "/" && (
              <button
                ...
                className={`flex items-center justify-center p-2 rounded-xl transition-all ${isDarkMode
                  ? "bg-white/10 text-white hover:bg-white/20"
                  : "bg-slate-800/10 text-slate-800 hover:bg-slate-800/20"
                  }`}
          ...
          <div className="absolute top-0 right-0 flex gap-3">
            {isLoggedIn ? (
              <>
                <Link
                  to="/profil"
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all ${location.pathname === "/profil"
                ...
                >
                  <div className={`w-6 h-6 rounded-full overflow-hidden flex items-center justify-center ${isDarkMode ? "bg-green-500/30" : "bg-green-500/20"
                    }`}>
          ...
          {/* Logo ve Başlık */}
          <h1 className={`text-4xl font-bold text-center mb-8 transition-colors ${isDarkMode ? "text-white" : "text-black"
            }`}>
            MonkMode
          </h1>
